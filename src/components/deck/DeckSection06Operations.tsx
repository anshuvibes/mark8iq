...
export default function DeckSection06Operations() {
  return (
    <section style={{ background: '#FFFFFF', minHeight: '100vh', display: 'flex', alignItems: 'center', padding: '80px 40px' }}>
      <div className="deck-two-col" style={{ display: 'flex', gap: 64, maxWidth: 1200, margin: '0 auto', width: '100%' }}>
        ...
        {/* Right */}
        <div style={{ flex: '0 0 55%', maxWidth: '55%', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16, alignContent: 'center' }}>
          {stats.map((stat, i) => (
            <motion.div
              key={i}
              style={{ background: '#EDF0F7', borderRadius: 12, padding: 28 }}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: '-80px' }}
              transition={{ duration: 0.6, ease: 'easeOut', delay: i * 0.1 }}
            >
              <p className="m8-h3-xl" style={{ color: '#8E59FF', marginBottom: 8 }}>{stat.value}</p>

      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ display: 'flex', gap: 20 }}>
          {people.map((person, i) => (
            ...
            >
              {person.isFounder && (
                <div style={{ position: 'absolute', top: 12, right: 12, padding: '2px 10px', borderRadius: 20, background: '#F5F0FF' }}>
                  <span className="m8-p6" style={{ color: '#8E59FF' }}>Founder</span>
                </div>
              )}
              <p className="m8-p3-medium" style={{ color: '#080D19', marginBottom: 4 }}>{person.name}</p>
              <p className="m8-p6" style={{ color: '#8E59FF', marginBottom: 16 }}>{person.role}</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {person.creds.map((c, j) => (
                  <p key={j} className="m8-p6" style={{ color: '#12182B' }}>{c}</p>
                ))}
              </div>
              {person.note && (
